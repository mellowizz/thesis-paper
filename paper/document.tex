...
conservation and monitoring of designated habitats by member states and for a report to be submitted every six years. The German federal state of
Rhineland-Palatinate has started to implement a thematically integrated
state-wide system with all relevant geo-data for plans and decision-making from
the local level to the EU levvel. Since collection of comprehensive,
good-quality environmental data through field recordings is expensive and time
consuming, remote sensing and other technologies can potentially solve these
problems. % (corbane et al 2015)
As a result the federal authorities in Rhineland-Paltinate started to implement
such a system in mid-2014 under the name NATFLO (Landscape objects from remotely
sensed data for nature conservancy). One of the goals of the projects is
...
differentiate between polygons come from the Rhineland Palatinate
\subsection{Ontology-based Reasoning}
Both the data mining algorithms, SEaTH and Decision Trees can  produce , produce a rule
set, they can be imported into an OWL ontology. so that a reasoner can perfrom
A-Box reasoning 
