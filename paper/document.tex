...
Geographic Object-Based Image Analaysis(GEOBIA) %and ontologies. Ontologien
% ha du hier noch nicht eingeführt.
to reduce the 
...
%vielleicht findest du hierfür noch eine Quelle
. An ontology, defined as ``an 
explicit specification of a conceptualization'' \citep{gruber1993} could help 
bridge the ``semantic gap'' and allow for better data transferability, 
predictability, interchangeability, knowledge and workflow management and 
logical consistency. %hier müssen Zitate rein! wie verbessert es
...
%% Wie besprochen: BIO_SOS würde ich nicht erwähnen. Die Zitate sollten drin
% bleiben. EOD-Ham auch!
...
interoperability has been demonstrated by \cite{Nieland2015}. 
...
\section{Method}
%Ich würde mit dem Methodenüberblick anfangen. Gib vielleicht eine kurze
% Einführung in den Methodenteil.
...
% therein is also explained.\\ Das könnte in die Einführung der Methode.
Figure \ref{fig_full_workflow} gives an overview\ldots %Ergänzen
%Ergänze, dass Daten unterschidlicher Ursprünge für die Refernenzdaten genutzt
% wurden und in einer Datenbank zusammengefasst werden. (Agriculture data,
% Forest data, Biotope data of Rhineland Palatinate)
...
% Verweise auf die anderen Kapitel (z.b. (1) ist beschrieben in 2.2 und 2.3 usw.). Hier muss noch rein: Woher kommen die
...
\multirow{4}{*}{Mesic grasslands} 
    & E2   & mesic & g/h & g/m & low/medium & 0 & 0 & 1\\
...
vocabulary.
%This process involved attaching indicators from EUNIS, newly created indicators
% by NATFLO and EAGLE to each biotope.

% Nimm NAtFLO raus. schreib eher etwas wie: detailed
% examintation showed that indicators that are taken directly from the EUNIS
...
The data mining module uses either the decision tree classifier (DT)
\footnote{http://scikit-learn.org/stable/modules/tree.html#tree} 
\citep{scikit-learn} or the Separability and Thresholds (SEaTH) algorithm \citep{Nussbaum2006} but the method can be extended to use other algorithms as long as the algorithm can generate a rule-set that can be
converted to OWL queries. 
...
OWLIndividals into the same ontology and the reasoner FaCT++ classifies all 
...
using the given features based on the \ldots. distance... One benefit of the
algorithm is that one does not need many training objects. In \cite{Nussbaum2006}, for example, the authors
...
very good results on several recent classification problems. (HIER MÜSSEN ZITATE
REIN)
...
%aerial images based on spectral information (NDVI% das darf man nicht NDVI
% nennen..
 and Bare Area Index) and
...
%und Geobasisinformation). % das ist so nicht richtig
