...
partially replaced with an automated \gls{rs} method that 
uses \gls{geobia} to reduce subjectivity, costs and time.
...
schema using earth observation data, existing
...
For
this pre-segmentation step two types of data were taken into account.  Digital
orthophotos (Year:2012-2013, resolution 0.2m) are used to create the Digital
Surface Model (\gls{dsm}) (resolution 0.5m) using automated stereo matching and
therefore represent the basis for all segmentation and classification
procedures. The \gls{dtm} and \gls{dem} was produced using LiDAR ASCII point 
clouds acquired
between 2003 and 2009 with a resolution of 0.2m.\\ The reference data (see
...
The actual classification process is based on two RapidEye Scenes (Year:2014)
and indices of the \gls{dt}M, DEM and orthophotos.
...
% indicator in particular is suprising as the data mining algorithms had access 
%to
% many different wetness indicators and morphological data. Since classifying
% \gls{eunis} level 2 grasslands requires information on wetness, the 
%classification
...
% performed poorly overall and the results show that the \gls{dt} classifier 
%achieves
% comparable results to the \gls{et} classifier for the indicators chosen. 
% Du vermischt results und discussion sehr stark. Bitte beschreibe hier
% lediglich die Ergebnisse OHNE Bewertung und Erkl??rung! Das kommt in die
% Discussion!
\begin{table}
    \centering
...
\section{Discussion}
%could be explained by the presence of these grassland
%classes. The presence of orchards 
