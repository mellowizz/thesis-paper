...
segmented by multi-spectral (B, G, R, NIR) orthophotos (see \ref{sec:usecase}) 
...
attaches landcover class labels to created polygons. This step is crucial as 
the quality of the segmentation determines the quality of the later 
...
the available input data for our test area were created (see 
...
formalization of the classes. This formalization can be written to an OWL
ontology, which is able to store complex logical connections in an OWL2 file. 
The logical consistency of the formalization can be verified by a reasoner 
performing T-Box (terminological component) reasoning.
...
that the formalization follows the same hierarchy is important and that 
subclasses are logical (grassland should not be a subclass of forest).
