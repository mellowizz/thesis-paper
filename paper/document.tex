...
mosses\ldots%bitte schau hier nochmal nach was alles in dem Datensatz vorkommt
...
% hier muss die Anzahl der Indices rein!‚
indices of the \gls{dtm}, \gls{dem} and orthophotos.
% Du solltest noch eine Liste aller berechneten Indices fuer den Anhang machen!
...
%Das brauchen wir nicht unbedingt.
\begin{figure}
...
might differ for that of a forest. %Diesen Satz verstehe ich nicht. Bitte
% umformulieren
...
%LC
%Das passt hier nicht. Der Absatz geht um semantic characterisation
...
only when it reaches a leaf node does it backtrack.%ist das ein richtiger Satz?!
%das verstehe ich ueberhaupt nicht. 
%Vielleicht: Starting with the root node,
% the algorithm visits all nodes of one branch of the generated decision tree as
% far as possible, before it contiues with diverging branches. Goal of this
% procedure is to come up to all reachable nodes.
 ...
 of false and right classified objects,%?was bedeutet nochmal Value =??? Bitte dazu
 % schreiben
...
The data was divided into 40\% for training and 60\% for validation. %Vielleicht
% anders rum?!
% Training 
...
the evolutionary search algorithm was used .%Bitte hier
% zitieren
...
database. 
% The indicators are then aggregated using SQL queries according to the
% formalized \gls{eunis} class definition to find which objects meet the 
% requirement. Das wuerde ich einfach mal unterschlagen..
...
been utilised successfully for several related research questions. % Bitte mach
% hier noch ein paar zitate rein.
