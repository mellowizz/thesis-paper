...
% ``micro theory'' ~\citep{Janowicz2012} which can then be used to map other 
theories
% by training the data mining algorithms for the new region and using the
% reasoner. We hope that once good test sites are taken for a region (for 
instance
% the federal state of Rhineland Palatinate) that the data mining algorithms 
will
...
% refers especially to the ones, which are derived from rather stable data 
sources
...
\end{table}
All described processes are based on a pre-segmentation based on GEOBIA.  For
...
and indices of the DTM, DEM and orthophotos.
...
%classes. The presence of orchards i KEINE ERKL??RUNG IN DIE ERGEBNISSE=> KOMMT
% IN DISSCUSSION
